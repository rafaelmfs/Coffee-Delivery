...
export function cartItemsReducer(state: CartItem[], actions: any) {
  switch (actions.type) {
    ...
    case actionTypes.REMOVE_ITEM_FROM_CART: {
      const newList = state.filter((item) => item.coffee.id !== actions.payload)
      return [...newList]
    }
    case actionTypes.INCREMENT_QUANTITY: {
      const cartItem = state.find((item) => item.coffee.id === actions.payload)
      if (cartItem) {
        ...
        const index = state.findIndex(
          (item) => item.coffee.id === actions.payload,
        )
        const newList = [
          ...state.slice(0, index),
          newItem,
